...
set cal4;
if default12 = . then default12 = 0; 
if default_cross12 = . then default_cross12 = 0;
/* Parametryzacja - bez zmian */
...
/* --- PRODUKT GŁÓWNY --- */
EL=0;
/* Dodano round dla bezpieczeństwa */
if round(default12, 1)=1 then EL=app_loan_amount*lgd;

/* ZMIANA NAZWY: installment -> installment_main */
installment_main = 0; 
if app_n_installments > 0 then 
    installment_main = app_loan_amount*apr*((1+apr)**app_n_installments)/
    (((1+apr)**app_n_installments)-1);

Income=0;
/* Używamy nowej nazwy installment_main */
if round(default12, 1)=0 then Income=app_n_installments*installment_main
...
/* --- PRODUKT CROSS --- */
EL_cross=0;
if round(default_cross12, 1)=1 then EL_cross=cross_app_loan_amount*lgd_cross;

/* ZMIANA NAZWY: installment -> installment_cross */
installment_cross = 0;
...
Income_cross=0;
/* Używamy nowej nazwy installment_cross */
if round(default_cross12, 1)=0 then Income_cross=cross_app_n_installments*installment_cross
...
/* Opcja 1: Jeśli chcesz widzieć WSZYSTKO z cal4 + wyniki, zakomentuj keep: */
/* keep aid cid product cross: pd: pr: year el: income: profit:; */

/* Opcja 2: Jeśli chcesz tylko wyniki, upewnij się że masz obie raty: */
keep aid cid product cross: pd: pr: year 
     el: income: profit: 
     installment_main installment_cross 
     app_loan_amount app_n_installments default12; /* Warto zachować dane wejściowe do weryfikacji */
run;
...
run;
%let pd_ins1=0.0402;
%let pd_ins2=0.6068; %let pr2=0.0614;
/*instalment loan then cash*/
